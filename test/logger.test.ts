...
describe("logger formatting", () => {
	...
	it("quotes complex values and strips control characters from messages", () => {
		assert.equal(
			formatLogLine(
				...
				"steam",
				"Retry\nstarted",
				{ error: "Logged in elsewhere", value: null },
				{ date: DATE },
			),
	...
	it("adds ANSI styling only when color is requested", () => {
		const informational = formatLogLine(
			"info",
			"steam",
			"Connected",
			{},
			{ color: true, date: DATE },
		);
		...
		assert.ok(informational.includes("\u001b[38;2;154;111;188mINFO \u001b[0m"));
		assert.ok(colored.includes("\u001b[31mERROR\u001b[0m"));
		assert.ok(
			informational.includes("\u001b[38;2;198;163;223m—           \u001b[0m"),
		);
		assert.ok(
			!formatLogLine(
				"error",
