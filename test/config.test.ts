...
describe("master key configuration", () => {
	it("prefers a configured secret file over an inline key", () => {
		...
		process.env.LINGER_MASTER_KEY =
			"inline-master-key-that-is-also-long-enough";
	...
	it("fails clearly instead of falling back when a configured secret file is unavailable", () => {
		...
		process.env.LINGER_MASTER_KEY =
			"inline-master-key-that-would-otherwise-work";

		assert.throws(
			() => loadConfig(),
			/LINGER_MASTER_KEY_FILE does not exist/iu,
		);
	});
