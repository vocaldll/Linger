...
describe("game library", () => {
  ...
  it("searches by game name or AppID", () => {
    assert.deepEqual(filterOwnedGames(GAMES, "BET").map((game) => game.appId), [20]);
    assert.deepEqual(filterOwnedGames(GAMES, "30").map((game) => game.appId), [30]);
  });
  ...
  it("pins manually entered AppIDs ahead of the sorted library", () => {
    ...
    assert.equal(entries.find((entry) => entry.appId === 20)?.manuallyAdded, false);
  });
