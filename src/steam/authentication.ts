...
export async function authenticate(
	...
	let machineAuthToken =
		method.type === "credentials" ? (method.machineAuthToken ?? null) : null;
	let settled = false;
	...
	const outcome = new Promise<AuthenticationResult>((resolve, reject) => {
		...
		session.on("timeout", () =>
			finish(() => reject(new Error("Steam Guard approval timed out"))),
		);
	...
	try {
		const response =
			method.type === "qr"
				...
				: await session.startWithCredentials({
						...
						...(method.machineAuthToken
							? { steamGuardMachineToken: method.machineAuthToken }
							: {}),
					});
		...
		if (response.actionRequired) {
			...
			const selected =
				onlyChoice !== undefined
					? ({ kind: "choice", choice: onlyChoice } as const)
					: await Promise.race([
							interaction
								.chooseGuard(choices)
								.then((choice) => ({ kind: "choice", choice }) as const),
							outcome.then(
								(result) => ({ kind: "authenticated", result }) as const,
							),
						]);
			...
			if (isCodeChoice(choice)) {
				...
				interaction.notify(
					"Approve the sign-in request using the email Steam sent you.",
				);
			}
		}
