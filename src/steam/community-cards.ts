...
export interface CardCommunity {
  discoverFarmableGames(cookies: readonly string[]): Promise<CardFarmingEntry[]>;
...
export function parseBadgePage(html: string, page: number): BadgePage {
  ...
  if (
    ...
    /there was an error (?:loading|processing)|temporarily unavailable/iu.test($.root().text())
  ) {
    throw new Error("Steam Community returned an error instead of a badges page");
  }
  const looksLikeBadgePage =
    rows.length > 0 || $(".profile_badges, .badges_sheet, .profile_badges_header").length > 0;
  ...
  rows.each((_index, element) => {
    ...
    const remainingMatch = row.find(".progress_info_bold").text().match(REMAINING_DROPS_PATTERN)
      ?? row.text().match(REMAINING_DROPS_PATTERN);
    if (!remainingMatch) {
      if (row.find(".badge_title_playgame").length > 0 && !ZERO_DROPS_PATTERN.test(row.text())) {
        throw new Error("A farmable Steam badge row did not contain a valid drop count");
      }
      return;
    }
  ...
  const hasNextPage = $("a.pagebtn")
    ...
    .some((element) => {
      ...
      try {
        return Number(new URL(href, "https://steamcommunity.com").searchParams.get("p")) === nextPage;
      } catch {
        return false;
      }
    });
...
export function parseRemainingDrops(html: string, appId: number): number {
  ...
  const match = emphasizedText.match(REMAINING_DROPS_PATTERN)
    ?? pageText.match(REMAINING_DROPS_PATTERN);
  if (match) {
    return Number(match[1]);
  }
  if (ZERO_DROPS_PATTERN.test(emphasizedText) || ZERO_DROPS_PATTERN.test(pageText)) {
    return 0;
  }

  const looksLikeCardPage =
    $(".badge_card_set_cards, .badge_gamecard_page, .gamecards_inventorylink, .badge_title").length > 0;
  throw new Error(
    looksLikeCardPage
...
export class SteamCommunityCardService implements CardCommunity {
  ...
  async discoverFarmableGames(cookies: readonly string[]): Promise<CardFarmingEntry[]> {
    ...
    for (let page = 1; page <= MAX_BADGE_PAGES; page += 1) {
      ...
      for (const entry of parsed.entries) {
        remainingByAppId.set(
          entry.appId,
          Math.max(entry.remainingDrops, remainingByAppId.get(entry.appId) ?? 0)
        );
      }
      if (!parsed.hasNextPage) {
        return [...remainingByAppId.entries()].map(([appId, remainingDrops]) => ({
          appId,
          remainingDrops
        }));
      }
  ...
  async getRemainingDrops(cookies: readonly string[], appId: number): Promise<number> {
    const response = await this.loadPage(
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      ...
      if (response.status === 429 || response.status >= 500) {
        throw new Error(`Steam Community request failed: ${response.status} ${response.statusText}`);
      }
      if (!response.ok) {
        throw new Error(`Steam Community request failed: ${response.status} ${response.statusText}`);
      }
