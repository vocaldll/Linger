...
function sanitizeMessage(message: string): string {
	// biome-ignore lint/suspicious/noControlCharactersInRegex: Sanitizing control characters is the purpose of this expression.
...
function formatFields(fields: LogFields, color: boolean): string {
	return Object.entries(fields)
		.filter(
			(entry): entry is [string, Exclude<LogFields[string], undefined>] =>
				entry[1] !== undefined,
		)
		.map(([key, value]) => {
			const label = `${key}=`;
			return color
				? `${DIM}${label}${RESET}${formatValue(value)}`
...
export function formatLogLine(
	...
	const account =
		typeof fields.account === "string" && fields.account
	...
	const details = formatFields(
		Object.fromEntries(
			Object.entries(fields).filter(([key]) => key !== "account"),
		),
		color,
	);
...
function write(
	...
	stream.write(
		`${formatLogLine(level, subsystem, message, fields, { color: shouldUseColor(stream) })}\n`,
	);
}
